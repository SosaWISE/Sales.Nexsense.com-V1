...
app.factory("Site", ['$http', '$window', '$timeout', 'Messaging', function($http, $window, $timeout, $msg) {
    ...
    var timezoneoffset = -Math.round((new Date()).getTimezoneOffset() / 60);
 	...
 	function SiteObject() {
    	...
    	this.signInDialogOnClick = function() {
    		self.signIn().then(function(data) {
    ...
    if (document.cookie) {
        ...
        for (var i=0; i<cookies.length; i++) {
            ...
            if (pairs[0] == 'SessionId')
            	...
            	siteobj.user.permissions = JSON.parse(pairs[1]);
        }
    }
        ...
        return {
            ...
            controller: function($scope, $attrs) {
                ...
                $scope.$on('setData', function() {
                    if ($scope.graph) {
                        $timeout(function() { $scope.graph.setData(graphMessage.data); }, 0); // we do this on a timeout so it gets thrown to the back of the execution queue (that is, AFTER the display of the new graph element)
                    }
                });
            },
	...
	return {
		...
		displayMessage: function(msg, isWarning, callbackFunction, isCancellable) {
		    ...
		    if (msgBox == null) {
		        ...
		        msgCancelButton.onclick = function() { $site.hideMessage.call($site) };
		...
		*/
		displayNotification: function(messageText) {
		    ...
		    __notificationTimeout = setTimeout($site.hideNotification, 5000);
		},
		/***
...
app.directive("header", ['Site', '$window', function($site, $window) {
	return {
		restrict: 'E',
		templateUrl: SITE_PATH + 'view/templates/header.html',
		link: function($scope, element, attrs) {
			$scope.$site = $site;
...
app.directive("footer", ['Site', function($site) {
	return {
		restrict: 'E',
		templateUrl: SITE_PATH + 'view/templates/footer.html',
	};
}]);


app.directive("signindialog", ['Site', '$window', 'Messaging', function($site, $window, $msg) {
	return {
		restrict: 'E',
		templateUrl: SITE_PATH + 'view/templates/signindialog.html',
		link: function($scope, element, attrs) {
			$scope.$site = $site;
