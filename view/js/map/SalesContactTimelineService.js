app.factory("SalesContactTimelineService", ['$http', '$rootScope', 'GraphMessage', 'Messaging', 'Site', function($http, $rootScope, graphMessage, $msg, $site) {
		...
		this.loadReport = function($scope) {
			...
			.success(function(data) {
				if (!data.success && data.require_signin) {
					$site.displaySignInDialog(function() { self.loadReport($scope); });
					return;
				}
				...
				if (data && data.results) {
					...
					if (days == 0) {
						histogram = [
							{name:'12pm - 1pm', hour:12, value:0},
							{name:'1pm - 2pm', hour:13, value:0},
							{name:'2pm - 3pm', hour:14, value:0},
							{name:'3pm - 4pm', hour:15, value:0},
							{name:'4pm - 5pm', hour:16, value:0},
							{name:'5pm - 6pm', hour:17, value:0},
							{name:'6pm - 7pm', hour:18, value:0},
							{name:'7pm - 8pm', hour:19, value:0},
						];
					}
					// If the date range is 31 days or less we'll use days for the histogram
						...
						histogram = [];
						var d = new Date(self.startTimestamp.getFullYear(), self.startTimestamp.getMonth(), self.startTimestamp.getDate());
						while (d <= self.endTimestamp) {
							histogram.push({name:MONTHS[d.getMonth()].abbr + " " + d.getDate(), day:d.getDate(), month:d.getMonth()+1, year:d.getFullYear(), value:0});
					...
					// Otherwise we'll use months for the histogram
						...
						d.setDate(1);
						while (d <= self.endTimestamp) {
							histogram.push({name:MONTHS[d.getMonth()].abbr + " " + d.getFullYear(), year:d.getFullYear(), month:d.getMonth()+1, value:0});
							//d.setTime(d.getTime() + 1 * 86400000);
							var y = d.getFullYear();
							var m = d.getMonth() + 1;
							if (m > 11) {
				...
				for (var i=0; i<self.rawData.length; i++) {
					...
					if (display) {
						...
						for (var j=0; j<filteredData.length; j++) {
							if ((!filteredData[j].year || (filteredData[j].year == self.rawData[i].yr))
								&& (!filteredData[j].month || (filteredData[j].month == self.rawData[i].mo))
								&& (!filteredData[j].day || (filteredData[j].day == self.rawData[i].dy))
								&& (!filteredData[j].hour || (filteredData[j].hour == self.rawData[i].hr))) {
