app.controller("MapPage", ['$scope', '$http', '$window', '$timeout', 'Site', 'Messaging', 'SalesContactService', 'SalesAreaService', 'SalesContactTimelineService', function($scope, $http, $window, $timeout, $site, $msg, SalesContact, SalesArea, ContactTimeline) {
	$scope.$site = $site;
    ...
    $timeout(function() { $scope.isLoading = false; }, 1000); // lets the animated menus of the map load beneath a nice loading screen
	...
	function loadMapData() {
		...
		function get_categories() {
			...
			$http.get(SITE_PATH + 'ng/Contact/get_categories')
				...
				if (data && data.results && data.results.length) {
					...
					for (var i=0; i<$scope.categories.length; i++) {
						...
						$scope.categories[i].icon = {
							url: IMG_PATH + 'map/markers/categories/' + $scope.categories[i].filename,
		...
		function get_category_icons() {
			$http.get(SITE_PATH + 'ng/Contact/get_category_icons')
			.success(function(data) {
				if (!data.success && data.require_signin) {
					console.log('require_signin');
			...
			$http.get(SITE_PATH + 'ng/SystemType/get_system_types')
			.success(function(data) {
				if (!data.success && data.require_signin) {
					console.log('require_signin');
				...
				if (data && data.results) {
					...
					for (var i=0; i<$scope.systemTypes.length; i++) {
						...
						$scope.systemTypes[i].icon = {
							url: IMG_PATH + 'map/markers/systems/' + $scope.systemTypes[i].filename,
							scaledSize: new google.maps.Size(24, 24),
							origin: new google.maps.Point(0, 0),
							anchor: new google.maps.Point(12, 12),
						};
		...
		function get_offices() {
			$http.get(SITE_PATH + 'ng/Office/get_offices')
			.success(function(data) {
				if (!data.success && data.require_signin) {
					console.log('require_signin');
				...
				if (data && data.results) {
					data.results.push({id:0,officeCity:'All offices',officeState:''});
		...
		function get_sales_reps() {
			$http.get(SITE_PATH + 'ng/SalesRep/get_sales_reps')
			.success(function(data) {
				if (!data.success && data.require_signin) {
					console.log('require_signin');
				...
				if (data && data.results) {
					data.results.push({id:0,officeId:0,firstName:'All reps',lastName:''});
	...
	$scope.getCategoryIconPath = function(ico) {
		return IMG_PATH + 'map/markers/categories/'+ico;
	}
...
/*	$scope.dialogBoxIsDisplayed = false;
	...
	$scope.currentSystemChanged = function() {
		if ($scope.iconmode == 'systemType') {
			...
			var theicon = genericContactIcon;
	...
	***/
	function initialize() {
		...
		if (navigator.geolocation) {
		    ...
		    }, {timeout: 15000, enableHighAccuracy:true, maximumAge:2000});
		}
	...
	***/
	if (navigator.geolocation) {
		setInterval(function() {
		    ...
		    }, {timeout: 9900, enableHighAccuracy:true, maximumAge:2000});
...
app.filter('salesRepFilterByOffice', function () {
