app.factory("SalesContactService", ['$http', '$timeout', '$compile', 'Messaging', 'Site', function($http, $timeout, $compile, $msg, $site) {
	...
	function SalesContactService() {
		...
		this.selectNewCategory = function(filename) {
			...
			this.newCategory.icon = {
				url: IMG_PATH + 'map/markers/categories/' + this.newCategory.filename,
		...
		$http.get(SITE_PATH + 'view/templates/infowindow.html')
		.success(function(data) {
			infowindowContent = data;
		}).error(function(data) {
			consol.log("Couldn't load InfoWindow template");
		});
		var compiledInfoWindowContent = undefined;

		var self = this;

		// Handle clicks on the map
		this.handleClick = function($scope, event, latLng) {
			if (/*!this.newContact || */this.newContact.marker == undefined) {
		...
		this.saveNewContact = function($scope) {
			...
			if (navigator.geolocation) {
				function save(ct, lat, lng) {
					$http.post(SITE_PATH + 'ng/Contact/save_contact', {
						id: ct.id,
						...
						notes: ct.notes,
						followup: ct.followup == undefined ? '' : ct.followup.formatTimestamp(),
						latitude: ct.lat,
						longitude: ct.lng,
						...
						if (!data.success && data.require_signin) {
							$site.displaySignInDialog(function() { self.saveNewContact($scope); });
							return;
						}
		...
		this.loadVisibleContacts = function($scope) {
			...
			if (zm >= 15) {
				...
				$http.get(SITE_PATH + 'ng/Contact/get_contacts_in_area/salesRepId=' + sr_id + '&officeId=' + o_id + '&minlat=' + sw.lat() + '&maxlat=' + ne.lat() + '&minlng=' + sw.lng() + '&maxlng=' + ne.lng()
				).success(function(data) {
					if (!data.success && data.require_signin) {
						$site.displaySignInDialog(function() { self.loadVisibleContacts($scope); });
						return;
					}
		...
		this.saveCategory = function($scope) {
			$http.post('ng/Contact/save_category', {
				...
				if (!data.success && data.require_signin) {
					$site.displaySignInDialog(function() { self.saveCategory($scope); });
					return;
				}
				...
				if (data && data.results) {
					...
					data.results.icon = {
						url: IMG_PATH + 'map/markers/categories/' + data.results.filename,
						scaledSize: new google.maps.Size(24, 24),
						origin: new google.maps.Point(0, 0),
						anchor: new google.maps.Point(12, 12),
					};
		...
		this.deleteCategory = function($scope, category) {
			$http.post('ng/Contact/delete_category', {
				...
				if (!data.success && data.require_signin) {
					$site.displaySignInDialog(function() { self.deleteCategory($scope, category); });
					return;
				}
				...
				if (data && data.results) {
					...
					for (var i=0; i<self.contacts.length; i++) {
						if (self.contacts[i].categoryId == category.id) {
							if (self.contacts[i].marker)
								self.contacts[i].marker.setIcon(genericContactIcon);
						}
					}
				}				
			}).error(function(data) {
