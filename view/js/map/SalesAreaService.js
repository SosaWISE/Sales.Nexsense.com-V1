app.factory("SalesAreaService", ['$http', 'Messaging', 'Site', function($http, $msg, $site) {
	...
	function SalesArea(map) {
		...
		this.commitPathChanges = function() {
			...
			for (var i=0; i<ps.getLength(); i++) {
				...
				for (var j=0; j<p.length; j++) {
					new_path.push({lat:p[j].lat(), lng:p[j].lng()});
				}
		...
		this.setClickHandler = function($scope, callback) {
			google.maps.event.addListener(polygon, 'click', function() { callback.call(self, $scope); } );
			var ps = polygon.getPaths();
			for (var i=0; i<ps.getLength(); i++) {
				google.maps.event.addListener(ps.getAt(i), 'set_at', function() { console.log('polygon changed'); });
			}
		}
		...
		this.setAreaEditable = function(editable) {
			polygon.setOptions({editable:editable});
		}
		...
		this.getCentroid = function() {
			...
			for (var c=0; c<pd[0].length; c++) {
				if (minlat == undefined || pd[0][c].lat < minlat)
					minlat = pd[0][c].lat;
				if (maxlat == undefined || pd[0][c].lat > maxlat)
					maxlat = pd[0][c].lat;
				if (minlng == undefined || pd[0][c].lng < minlng)
					minlng = pd[0][c].lng;
				if (maxlng == undefined || pd[0][c].lng > maxlng)
					maxlng = pd[0][c].lng;
	...
	function SalesAreaServiceObj() {
		...
		this.saveCurrentSalesArea = function($scope) {
			...
			// try to get lat lon
			$http.post(SITE_PATH + 'ng/SalesArea/save_area', {
				...
				if (!data.success && data.require_signin) {
					$site.displaySignInDialog(function() { self.saveCurrentSalesArea($scope); });
					return;
				}
		...
		this.deleteCurrentSalesArea = function($scope) {
			...
			function deleteArea() {
				$http.post(SITE_PATH + "ng/SalesArea/delete_sales_area", {
					areaId: sa.id 
				}).success(function(data) {
					if (!data.success && data.require_signin) {
						$site.displaySignInDialog(function() { self.deleteCurrentSalesArea($scope); });
						return;
					}
		...
		this.loadVisibleAreas = function($scope) {
			...
			if (zm >= 13) {
				...
				// load from server
				$http.get(SITE_PATH + 'ng/SalesArea/get_sales_areas/salesRepId=' + sr_id + '&officeId=' + o_id + '&minlat=' + sw.lat() + '&maxlat=' + ne.lat() + '&minlng=' + sw.lng() + '&maxlng=' + ne.lng())
				.success(function(data) {
					if (!data.success && data.require_signin) {
						$site.displaySignInDialog(function() { self.loadVisibleAreas($scope); });
						return;
					}
